...
void sEn_WriteReg(uint8_t devAddr, uint8_t regAddr, uint16_t data)
	{
	...
	sEn_SendDebugMsg(sEn_DBG_MSG_W, devAddr, i2cdata[0], (i2cdata[1] <<8 | i2cdata[2]), i2cdata[3], 0x00);
	...
	HAL_I2C_Master_Transmit(&hi2c1, (devAddr << 1), i2cdata, 4, 0xFFFF);
	HAL_Delay(10);
	sEn_SendDebugMsg(sEn_DBG_MSG_W, devAddr, i2cdata[0], data, i2cdata[3], 0x00);

  }

uint16_t sEn_ReadReg(uint8_t devAddr, uint8_t regAddr, uint8_t dbg_lvl)
{
	...
	if(dbg_lvl == sEn_DBG_ON)	sEn_SendDebugMsg(sEn_DBG_MSG_R, devAddr, regAddr, data, in_buff[2], crc);

	//HAL_Delay(1);
	return data;
}

float sEn_ReadTemp(uint8_t devAddr, uint8_t regAddr)
	{

	float temp;
	uint16_t data;

	data = sEn_ReadReg(devAddr, regAddr, sEn_DBG_OFF);
...
void sEn_SendDebugMsg(uint8_t op_type, uint8_t devAddr, uint8_t regAddr, uint16_t data, uint8_t crc_in, uint8_t crc_calc)
	{

	if(op_type == sEn_DBG_MSG_W)
		{
		snprintf(temp_buff, sizeof(temp_buff), "W Dev: 0x%02X, Reg: 0x%02X, Data: 0x%04X, CRC8_calc:0x%02X\r\n", devAddr, regAddr, data, crc_calc);
		CDC_Transmit_FS(temp_buff, strlen((const char *)temp_buff));
	  }
	else if (op_type == sEn_DBG_MSG_R)
		{
